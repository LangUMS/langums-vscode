...
var _grp = require('./grp');

var _grp2 = _interopRequireDefault(_grp);

var _sprites = require('./sprites.json');

var _sprites2 = _interopRequireDefault(_sprites);

var _units = require('./units.json');
...
const isResource = unitId => unitId >= UNIT_ID_MINERAL1 && unitId <= UNIT_ID_MINERAL3 || unitId === UNIT_ID_GEYSER;

const isCritter = unitId => unitId === UNIT_ID_RHYNADON || unitId === UNIT_ID_BENGALAAS || unitId === UNIT_ID_RAGNASAUR || unitId === UNIT_ID_SCANTID || unitId === UNIT_ID_URSADON || unitId === UNIT_ID_KAKARU;

const NEUTRAL_PLAYER = 11;
// The colors are stored in tunit.pcx
const PLAYER_COLORS = [[0x6f, 0x17, 0x17, 0x62, 0x5e, 0xab, 0xaa, 0xa8], [0xa5, 0xa2, 0xa2, 0x2d, 0xa0, 0x2a, 0x29, 0x28], [0x9f, 0x9e, 0x9e, 0x9e, 0x9d, 0x9d, 0x9d, 0xb6], [0xa4, 0xa4, 0xa4, 0xa3, 0xa1, 0xa1, 0xa1, 0x5a], [0x9c, 0xb3, 0x1c, 0x1a, 0x15, 0x12, 0x59, 0x56], [0x13, 0x12, 0x12, 0x5c, 0x5c, 0x59, 0x58, 0x56], [0x54, 0x53, 0x51, 0x4e, 0x96, 0x92, 0x90, 0x43], [0x87, 0xa7, 0xa6, 0x81, 0x65, 0x61, 0x5c, 0x56], [0xb9, 0xb8, 0xb8, 0xb8, 0xb7, 0xb7, 0xb7, 0xb6], [0x88, 0x88, 0x84, 0x83, 0x81, 0x93, 0x63, 0x44], [0x67, 0x72, 0x83, 0x82, 0x6b, 0x19, 0x16, 0x12], [0x33, 0x33, 0x31, 0x31, 0x2d, 0x2d, 0xa0, 0x29]];

const TILESET_NAMES = ['badlands', 'platform', 'install', 'ashworld', 'jungle', 'desert', 'ice', 'twilight'];
const TILESET_NICE_NAMES = ['Badlands', 'Space', 'Installation', 'Ashworld', 'Jungle', 'Desert', 'Ice', 'Twilight'];
...
class FileAccess {
  constructor(cb) {
    ...
    this._sprites = new _grp2.default(_units2.default, _sprites2.default);
  }
  ...
  tileset(id) {
    ...
    return _asyncToGenerator(function* () {
      return yield _this._tilesets.tileset(id, _this.read);
    })();
  }
  ...
  unit(id) {
    ...
    return _asyncToGenerator(function* () {
      return yield _this2._sprites.unit(id, _this2.read);
    })();
  }
  ...
  sprite(id) {
    ...
    return _asyncToGenerator(function* () {
      return yield _this3._sprites.sprite(id, _this3.read);
    })();
  }
}
...
class StrSection {
  ...
  get(index) {
    ...
    return _iconvLite2.default.decode(this._data.slice(offset, end), this.encoding);
  }
  ...
  _determineEncoding(usedStrings) {
    ...
    for (const string of usedStrings) {
      ...
      const korean = _iconvLite2.default.decode(raw, 'cp949');
      let hangulChars = 0;
      let otherNonAscii = 0;
...
class Chk {
  constructor(buf, options) {
    ...
    [this.title, this.description] = this._parseScenarioProperties(sections.section('SPRP')).map(index => this._strings.get(index));
    ...
    [this.forces, this._maxMeleePlayers] = this._parsePlayers(forceSection, sections.section('OWNR'), sections.section('SIDE'));
    this._tiles = sections.section('MTXM');

    [this.units, this.sprites] = this._parseUnits(sections.section('UNIT'), sections.section('THG2'));
	
	if(sections.get('MRGN') === undefined)
      this.locations = [];
    else
  ...
  static createStream(callback) {
    const stream = new _stream.Duplex({ readableObjectMode: true });
    const buf = new _bl2.default();
  ...
  maxPlayers(ums) {
    if (ums) {
      return this.forces.reduce((accum, force) => accum + force.players.filter(player => !player.computer).length, 0);
    } else {
  ...
  static fsFileAccess(directory) {
    return Chk.customFileAccess(fname => new Promise((res, rej) => {
      _fs2.default.createReadStream(directory + '/' + fname.replace(/\\/g, '/')).pipe(new _bl2.default((err, buf) => {
        if (err) {
          rej(err);
        } else {
          res(buf);
  ...
  // }
  // ```
  image(fileAccess, width, height, options) {
    var _this4 = this;
    ...
    return _asyncToGenerator(function* () {
      ...
      const tileset = yield fileAccess.tileset(_this4.tileset);
      _this4._renderTerrain(tileset, out, width, height);

      const mapWidthPixels = _this4.size[0] * 32;
      const mapHeightPixels = _this4.size[1] * 32;
      const scaleX = width / mapWidthPixels;
      const scaleY = height / mapHeightPixels;
      const palette = tileset.palette;
      yield _this4._renderSprites(fileAccess, palette, out, width, height, scaleX, scaleY, opts);
      return out;
    })();
  }
  ...
  _renderSprites(fileAccess, palette, surface, width, height, scaleX, scaleY, opts) {
    var _this5 = this;

    return _asyncToGenerator(function* () {
      const startLocationCheck = function (u) {
        return u.unitId !== UNIT_ID_START_LOCATION || opts.startLocations;
      };
      const meleeCheck = function (u) {
        if (!opts.melee) {
          return true;
        }
        if (u.sprite) {
      ...
      for (const unit of _this5.units) {
        ...
        const sprite = yield fileAccess.unit(drawnUnitId);
...
class Tilesets {
  ...
  tileset(id, readCb) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const tileset = _this6._tilesets[id];
      if (tileset) {
        return tileset;
      }

      if (!_this6._incompeleteTilesets[id]) {
        _this6._addFiles(id, readCb);
      }
      yield _this6._incompeleteTilesets[id];
      return _this6._tilesets[id];
    })();
  }
